...
public class OaidHelper implements IIdentifierListener {
    ...
    public static final int HELPER_VERSION_CODE = 20230516; // MdidSdkHelper版本号
    private AppIdsUpdater appIdsUpdater = new AppIdsUpdater();
    private boolean isCertInit = false;
    private boolean isArchSupport = false;
    public boolean isSDKLogOn = false;          // TODO （1）设置 是否开启sdk日志
    public static final String ASSET_FILE_NAME_CERT = "com.fantablade.watergun.cert.pem";  // TODO （2）设置 asset证书文件名
    ...
    public OaidHelper(){
        ...
        Log.w(TAG, loadLibrary("msaoaidsec"));
    ...
    public static byte[] aes(byte[] input, byte[] key, int mode) throws UnsupportedEncodingException {
        try {
    ...
    public static String aesEncrypt(String input, String key) throws UnsupportedEncodingException {
//      if (StringUtil.isNullOrEmpty(input)) {
//          return "";
//      }
        byte[] aes = aes(input.getBytes("UTF-8"), key.getBytes("UTF-8"), Cipher.ENCRYPT_MODE);
        String result = Base64.encodeToString(aes, Base64.DEFAULT);
