...
using FantaBlade.Platform.Internal.Jwt;
using UnityEngine;
...
namespace FantaBlade.Platform.Internal
{
    internal class AuthManager
    {
        ...
        public FantaBladePlatform.LoginChannel curLoginChannel;
        ...
        public void QuickLogin()
        {
            ...
            PlayerPrefs.SetInt("loginChannel", (int)FantaBladePlatform.LoginChannel.CHANNEL_OFFICIAL);
        ...
        public void LoginByThird(string authCode, FantaBladePlatform.LoginChannel loginChannel)
        {
            IsLoggingIn = true;
            var form = new Dictionary<string, string>
            {
        ...
        public void LoginThird(FantaBladePlatform.LoginChannel channel)
        {
            // 获取第三方access_token/auth_code
            curLoginChannel = channel;
            PlayerPrefs.SetInt("loginChannel", (int)channel);
            SdkManager.NativeApi.Login((int)curLoginChannel);
        }

        public void CheckIsActication(string activationCode, Action<bool> activatedCallback)
        {
            if (FantaBladePlatform.NeedActivation)
            {
                SdkManager.Ui.Dialog.ShowLoading();
        ...
        {
            FantaBladePlatform.OnLoginSuccess(Token);
            //效果不好，先屏蔽
        ...
        public void Logout()
        {
            Token = null;
            IsVerify = false;
            FantaBladePlatform.OnLogoutSuccess();
#if UNITY_ANDROID && !UNITY_EDITOR
            if (SdkManager.UseAndroidNativeApi)
            {
        ...
        public void OnSDKLoginFinish(bool success, string authCode)
        {
            ...
            if (success)
            {
                if (curLoginChannel == FantaBladePlatform.LoginChannel.CHANNEL_MOBILE)
                {
            ...
            }else if (curLoginChannel == FantaBladePlatform.LoginChannel.CHANNEL_MOBILE)
            {
                SdkManager.Ui.ShowLogin();
            }
            else if (authCode == "UserCancel")
            {
                FantaBladePlatform.OnLoginCancel();
            }
            else if (authCode == "iOSNotSupport")
            {
                SdkManager.Ui.Dialog.Show("系统版本不支持,请使用其他方式或升级系统", "ok");
                FantaBladePlatform.OnLoginFailure(authCode);
            }
            else
            {
                FantaBladePlatform.OnLoginFailure(authCode);
